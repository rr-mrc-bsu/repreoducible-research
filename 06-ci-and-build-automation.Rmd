...
knitr::opts_chunk$set(fig.align='center', echo=FALSE, fig.path="Figures/", 
               cache=FALSE)
```
...
in your workflow, thereby simplifying your the whole process. Automation can be 
done on several levels. Here, we will mainly discuss two facets of build 
automation: automating your project pipeline using Makefiles and automatically 
building and checking your software using continuous integration tools.
...
Consider the more complicated example in Figure \@ref(fig:workflow-diagram). 
Re-running, -building, and -compiling all the files after we made a change to 
the functions.R file would be a tedious task. Ideally, we would like to have a 
command that re-runs/compiles/builds the different files everytime an upstream 
change is made. This is exactly what the GNU software Make 
does.
